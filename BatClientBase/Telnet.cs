...
namespace BatMud.BatClientBase
{
	...
	public class Telnet
	{
		...
		{
#if TELNET_DEBUG
			BatConsole.WriteLine(((TelnetCodes)data[pos]).ToString() + " ");
#endif
			if(pos+1 >= len)
			{
#if TELNET_DEBUG
				BatConsole.WriteLine("Uncomplete telnet sequence");
#endif
			...
			{
				BatConsole.WriteLine("Unknown telnet code " + data[pos + 1]);
				// consume IAC
				pos++;
				scanned = pos;
				return;
...
#if TELNET_DEBUG
			BatConsole.WriteLine(telCode.ToString());
#endif
				...
				telCode == TelnetCodes.WILL || telCode == TelnetCodes.WONT)
			{
				...
				{
					BatConsole.WriteLine("Unknown telnet option " + data[pos + 2]);
					// consume IAC
					pos++;
					scanned = pos;
					return;
...
#if TELNET_DEBUG
				BatConsole.WriteLine(telOpt.ToString());
#endif
				if(telnetEvent != null)
				{
