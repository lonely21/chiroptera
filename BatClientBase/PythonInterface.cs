...
namespace BatMud.BatClientBase
{
	static public class PythonInterface
	{
		...
		public static IBatConsole Console
		{
		...
		{
			string batPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), "BatClient");
#if DEBUG
			string[] paths = new string[] { batPath, "../../../scripts/std" };
#else
			string[] paths = new string[] { batPath, "std" };
#endif
		...
		public static void RunScript(string file)
		{
			...
			if (filePath == null)
			{
				BatConsole.WriteLine("Script not found: {0}", file);
				return;
			}
		...
		static void RunBatScript(string filePath)
		{
			...
			foreach (string line in lines)
			{
				if (line.Length > 0 && line[0] != '#')
				{
					...
					if (res != 0)
					{
						BatConsole.WriteLine("{0}({1}): Command failed", file, lineNum);
						break;
					}
				}
...
";
			...
			{
				BatConsole.WriteError("Error running script " + file, e, file);
			}
		}
